...
public abstract class StatefulAgent extends ApeAgent implements GraphListener {
	...
	public void updateModel(Model newModel) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (action.isModelAction() && action.requireTarget()) {
				...
				actionHistory.set(i,
						new ActionRecord(actionPair.clockTimestamp, actionPair.agentTimestamp, action, guiAction));
			}
		}
	}
	...
	protected ModelAction selectNewActionFromBuffer() {
		...
		if (expectedNewState != null && !expectedNewState.equals(newState)) {
			...
			if (!actionBuffer.isEmpty() && currentStateTransition.isSameActivity()) {
				...
				if (relocatedAction == null) {
					...
					int throttle = Math.max(relocatedAction.getThrottle(), actionBuffer.peekFirst().getThrottle()); // transition
																													// throttle
					Logger.iformat("Buffer action throttle: original: %d, tracked: %d", relocatedAction.getThrottle(),
							actionBuffer.peekFirst().getThrottle());
		...
		Logger.iformat("Buffer action throttle: original: %d, tracked: %d", action.getThrottle(),
				actionBuffer.peekFirst().getThrottle());
		action.setThrottle(throttle);
		return action;
	}
...
//		System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
		...
		Logger.iprintln("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
		Logger.iprintln(newState.toString());
		if (newState.toString().contains("LoginActivity")){
			System.out.print("Please manual login and press enter");
			pause(); // '중지'시키기
...
//		if (newState.toString().contains("LoginActivity") || newState.toString().contains("EmailLoginActivity")) {
//			if (count < 2) {
//				count = count + 1;
//				Logger.iprintln("count : " + count);
//			}else {
...
//		}
	...
	protected void preEvolveModel() {
		...
		}
		{
			...
			long end = SystemClock.elapsedRealtimeNanos();
			Logger.iformat("Pre-checking under-abstracted states takes %d ms",
		...
		}
		{
			...
			checkUnderAbstractedState();
			long end = SystemClock.elapsedRealtimeNanos();
			Logger.iformat("Post-checking under-abstracted states takes %d ms",
					TimeUnit.NANOSECONDS.toMillis(end - begin));
		}
	}
	...
	protected void adjustActionsByGUITree() {
		...
		for (ModelAction action : newState.getActions()) {
			...
			if (!action.isSaturated()) {
				...
				int size = nodes.size();
				if (size > 1) {
					priority += Math.min(size, maxExtraPriorityAliasedActions)
							* getActionBasePriority(action.getType());
				}
			}
